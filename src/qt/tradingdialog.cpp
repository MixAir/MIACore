        ...
        model(0)
{
    ...
    ui->MIAAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaMIALabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "MIA(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "MIA(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(MIA)"<<"TOTAL COST(BTC)");
...
void tradingDialog::UpdaterFunction(){
    //MIAst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
    ...
    QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/MIA_BTC");
    return Response;
}

QString tradingDialog::GetOrderBook(){

    QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/MIA_BTC");
    return Response;
}
...
void tradingDialog::GetMarketHistory(){
    ...
    sendRequest1(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/MIA_BTC"), f);
}
...
QString tradingDialog::BuyMIA(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";

    QJsonObject stats_obj;
    stats_obj["Market"] = "MIA/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellMIA(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"MIA\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>MIA Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> MIA");
...
void tradingDialog::AskInfoInsertSlot(int row, int col){
    ...
    QString UnitsMIAString = ui->AsksTable->model()->data(ui->AsksTable->model()->index(row,1)).toString();
    ui->SellBidPriceEdit->setText(SellBidPriceString);
    ui->UnitsInputMIA->setText(UnitsMIAString);
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->MIASupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(MIASupply,'i',8) + "</span><b> MIA</b>");
    ...
    ui->MIADemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(MIADemand,'i',8) + "</span><b> MIA</b>");
...
void tradingDialog::ParseAndPopulatePriceChart(QString Response){
    ...
    derPlot->yAxis->setLabel("MIA Price in BTC");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
        case 0:    //buy tab is active
            ...
            f = std::bind(&tradingDialog::showBalanceOfMIAOnTradingTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "MIA" + QString("\"}"));
            ...
            sendRequest1("https://www.cryptopia.co.nz/api/GetMarketOrders/MIA_BTC", f);
        ...
        case 1: //Cross send tab active
            ...
            f = std::bind(&tradingDialog::showBalanceOfMIAOnSendTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "MIA" + QString("\"}"));
        ...
        case 5://show balance tab
            ...
            f = std::bind(&tradingDialog::showBalanceOfMIAOnBalanceTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "MIA" + QString("\"}"));
...
void tradingDialog::showBalanceOfMIAOnTradingTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->MIAAvailableLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfMIAOnSendTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->CryptopiaMIALabel, Response);
    }
}
...
void tradingDialog::showBalanceOfMIAOnBalanceTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->MIABalanceLabel,*ui->MIAAvailableLabel_2,*ui->MIAPendingLabel, QString::fromUtf8("MIA"),Response);
    }
}
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputMIA->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("MIA");
    ...
    double AvailableMIA = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling MIA available balance
    QString responseA = GetBalance("MIA");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableMIA = ResultObject["Available"].toDouble();

    ui->UnitsInputMIA->setText(str.number(AvailableMIA,'i',8));
}
...
{
    double Quantity = ui->CryptopiaMIALabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableMIA,'i',8));
}
...
void tradingDialog::on_BuyMIA_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyMIA(Order,Quantity,Rate);
...
void tradingDialog::on_SellMIABTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputMIA->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellMIA(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_CSUnitsBtn_clicked()
{
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        if ( ((Quantity / x) - y) > 0 )
        {
            ...
            QString SellResponse = SellMIA(Order,y,x);
            ...
            QString SellResponse = SellMIA(Order,(Quantity / x),x);
            QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
            QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
            ...
            if (SellResponseObject["Success"].toBool() == false){
                ...
                if (ResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" MIA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
                } else if (ResponseObject["Success"].toBool() == true){
                    QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" MIA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                }
...
void tradingDialog::on_UnitsInputMIA_textChanged(const QString &arg1)
{
    CalculateSellCostLabel(); //update cost
}
