...
const std::string DEFAULT_MIA_CONTROL = "127.0.0.1:9051";
/** Lux cookie size (from control-spec.txt) */
static const int MIA_COOKIE_SIZE = 32;
/** Size of client/server nonce for SAFECOOKIE */
static const int MIA_NONCE_SIZE = 32;
/** For computing serverHash in SAFECOOKIE */
static const std::string MIA_SAFE_SERVERKEY = "Lux safe cookie authentication server-to-controller hash";
/** For computing clientHash in SAFECOOKIE */
static const std::string MIA_SAFE_CLIENTKEY = "Lux safe cookie authentication controller-to-server hash";
/** Exponential backoff configuration - initial timeout in seconds */
static const float RECONNECT_TIMEOUT_START = 1.0;
/** Exponential backoff configuration - growth factor */
static const float RECONNECT_TIMEOUT_EXP = 1.5;
/** Maximum length for lines received on LuxControlConnection.
 * mia-control-spec.txt mentions that there is explicitly no limit defined to line length,
 ...
 * Speaks the SMTP-like protocol as defined in miaspec/control-spec.txt
 */
...
void LuxControlConnection::readcb(struct bufferevent *bev, void *ctx)
{
        ...
        if (ch == ' ') {
            ...
            if (self->message.code >= 600) {
                ...
                if (!self->reply_handlers.empty()) {
                    ...
                    LogPrint("mia", "mia: Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
    ...
    if (evbuffer_get_length(input) > MAX_LINE_LENGTH) {
        LogPrintf("mia: Disconnecting because MAX_LINE_LENGTH exceeded\n");
        self->Disconnect();
    }
}
...
void LuxControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrint("mia", "mia: Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrint("mia", "mia: Error connecting to Lux control socket\n");
        } else {
            LogPrint("mia", "mia: End of stream\n");
        }
        self->Disconnect();
...
bool LuxControlConnection::Connect(const std::string &target, const ConnectionCB& _connected, const ConnectionCB&  _disconnected)
{
    ...
    if (evutil_parse_sockaddr_port(target.c_str(),
        ...
        LogPrintf("mia: Error parsing socket address %s\n", target);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, (struct sockaddr*)&connect_to_addr, connect_to_addrlen) < 0) {
        LogPrintf("mia: Error connecting to address %s\n", target);
        return false;
    }
    return true;
}
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!reconnect_ev)
        LogPrintf("mia: Failed to create event for reconnection: out of memory?\n");
    ...
    if (!conn.Connect(_target, boost::bind(&LuxController::connected_cb, this, _1),
        ...
        LogPrintf("mia: Initiating connection to Lux control port %s failed\n", _target);
    }
    // Read service private key if cached
    std::pair<bool,std::string> pkf = ReadBinaryFile(GetPrivateKeyFile());
    if (pkf.first) {
        LogPrint("mia", "mia: Reading cached private key from %s\n", GetPrivateKeyFile());
...
{
    if (reply.code == 250) {
        LogPrint("mia", "mia: ADD_ONION successful\n");
        ...
        if (service_id.empty()) {
            LogPrintf("mia: Error parsing ADD_ONION parameters:\n");
            for (const std::string &s : reply.lines) {
                LogPrintf("    %s\n", SanitizeString(s));
            }
            return;
        ...
        LogPrintf("mia: Got service ID %s, advertising service %s\n", service_id, service.ToString());
        if (WriteBinaryFile(GetPrivateKeyFile(), private_key)) {
            LogPrint("mia", "mia: Cached service private key to %s\n", GetPrivateKeyFile());
        } else {
            LogPrintf("mia: Error writing service private key to %s\n", GetPrivateKeyFile());
        }
        AddLocal(service, LOCAL_MANUAL);
        // ... onion requested - keep connection open
    } else if (reply.code == 510) { // 510 Unrecognized command
        LogPrintf("mia: Add onion failed with unrecognized command (You probably need to upgrade Lux)\n");
    } else {
        LogPrintf("mia: Add onion failed; error code %d\n", reply.code);
    }
}
...
{
    if (reply.code == 250) {
        LogPrint("mia", "mia: Authentication successful\n");
        ...
        LogPrintf("mia: Authentication failed\n");
    }
}
...
{
    if (reply.code == 250) {
        LogPrint("mia", "mia: SAFECOOKIE authentication challenge successful\n");
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            if (m.empty()) {
                LogPrintf("mia: Error parsing AUTHCHALLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            std::vector<uint8_t> serverHash = ParseHex(m["SERVERHASH"]);
            std::vector<uint8_t> serverNonce = ParseHex(m["SERVERNONCE"]);
            LogPrint("mia", "mia: AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
            if (serverNonce.size() != 32) {
                LogPrintf("mia: ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }

            std::vector<uint8_t> computedServerHash = ComputeResponse(MIA_SAFE_SERVERKEY, cookie, clientNonce, serverNonce);
            if (computedServerHash != serverHash) {
                LogPrintf("mia: ServerHash %s does not match expected ServerHash %s\n", HexStr(serverHash), HexStr(computedServerHash));
                return;
            }

            std::vector<uint8_t> computedClientHash = ComputeResponse(MIA_SAFE_CLIENTKEY, cookie, clientNonce, serverNonce);
            _conn.Command("AUTHENTICATE " + HexStr(computedClientHash), boost::bind(&LuxController::auth_cb, this, _1, _2));
        } else {
            LogPrintf("mia: Invalid reply to AUTHCHALLENGE\n");
        }
    } else {
        LogPrintf("mia: SAFECOOKIE authentication challenge failed\n");
    }
}
...
void LuxController::protocolinfo_cb(LuxControlConnection& _conn, const LuxControlReply& reply)
{
    if (reply.code == 250) {
        ...
        /*
         * 250-AUTH METHODS=COOKIE,SAFECOOKIE COOKIEFILE="/home/x/.mia/control_auth_cookie"
        ...
        for (const std::string &s : reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Lux")) != m.end()) {
                    LogPrint("mia", "mia: Connected to Lux version %s\n", i->second);
                }
            }
        }
        for (const std::string &s : methods) {
            LogPrint("mia", "mia: Supported authentication method: %s\n", s);
        }
        // Prefer NULL, otherwise SAFECOOKIE. If a password is provided, use HASHEDPASSWORD
        /* Authentication:
         *   cookie:   hex-encoded ~/.mia/control_auth_cookie
         *   password: "password"
         */
        std::string miapassword = GetArg("-miapassword", "");
        if (!miapassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrint("mia", "mia: Using HASHEDPASSWORD authentication\n");
                boost::replace_all(miapassword, "\"", "\\\"");
                _conn.Command("AUTHENTICATE \"" + miapassword + "\"", boost::bind(&LuxController::auth_cb, this, _1, _2));
            } else {
                LogPrintf("mia: Password provided with -miapassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint("mia", "mia: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", boost::bind(&LuxController::auth_cb, this, _1, _2));
        } else if (methods.count("SAFECOOKIE")) {
            // Cookie: hexdump -e '32/1 "%02x""\n"'  ~/.mia/control_auth_cookie
            LogPrint("mia", "mia: Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);
            std::pair<bool,std::string> status_cookie = ReadBinaryFile(cookiefile, MIA_COOKIE_SIZE);
            if (status_cookie.first && status_cookie.second.size() == MIA_COOKIE_SIZE) {
                // _conn.Command("AUTHENTICATE " + HexStr(status_cookie.second), boost::bind(&LuxController::auth_cb, this, _1, _2));
                cookie = std::vector<uint8_t>(status_cookie.second.begin(), status_cookie.second.end());
                clientNonce = std::vector<uint8_t>(MIA_NONCE_SIZE, 0);
                GetRandBytes(&clientNonce[0], MIA_NONCE_SIZE);
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&LuxController::authchallenge_cb, this, _1, _2));
            } else {
                if (status_cookie.first) {
                    LogPrintf("mia: Authentication cookie %s is not exactly %i bytes, as is required by the spec\n", cookiefile, MIA_COOKIE_SIZE);
                } else {
                    LogPrintf("mia: Authentication cookie %s could not be opened (check permissions)\n", cookiefile);
                }
            }
        } else if (methods.count("HASHEDPASSWORD")) {
            LogPrintf("mia: The only supported authentication mechanism left is password, but no password provided with -miapassword\n");
        } else {
            LogPrintf("mia: No supported authentication method\n");
        }
    } else {
        LogPrintf("mia: Requesting protocol info failed\n");
    }
}
...
void LuxController::connected_cb(LuxControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", boost::bind(&LuxController::protocolinfo_cb, this, _1, _2)))
        LogPrintf("mia: Error sending initial protocolinfo command\n");
}
...
void LuxController::disconnected_cb(LuxControlConnection& _conn)
{
    ...
    LogPrint("mia", "mia: Not connected to Lux control port %s, trying to reconnect\n", target);
...
void LuxController::Reconnect()
{
    ...
    if (!conn.Connect(target, boost::bind(&LuxController::connected_cb, this, _1),
         boost::bind(&LuxController::disconnected_cb, this, _1) )) {
        LogPrintf("mia: Re-initiating connection to Lux control port %s failed\n", target);
    }
}
...
static boost::thread miaControlThread;

static void LuxControlThread()
{
    LuxController ctrl(gBase, GetArg("-miacontrol", DEFAULT_MIA_CONTROL));
...
void StartLuxControl(boost::thread_group& threadGroup/*, CScheduler& scheduler*/)
{
    ...
    if (!gBase) {
        LogPrintf("mia: Unable to create event_base\n");
        return;
    }

    miaControlThread = boost::thread(boost::bind(&TraceThread<void (*)()>, "miacontrol", &LuxControlThread));
}

void InterruptLuxControl()
{
    if (gBase) {
        LogPrintf("mia: Thread interrupt\n");
...
void StopLuxControl()
{
    ...
    if (gBase) {
        ...
        miaControlThread.try_join_for(boost::chrono::seconds(1));
#else
        miaControlThread.timed_join(boost::posix_time::seconds(1));
#endif
