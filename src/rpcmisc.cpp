...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mia balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized mia balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in mia/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in mia/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    LuxDGP miaDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)miaDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)miaDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)miaDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"miaaddress\"\n"
            "\nReturn information about the given mia address.\n"
            "\nArguments:\n"
            "1. \"miaaddress\"     (string, required) The mia address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"miaaddress\", (string) The mia address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MIA address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are mia addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mia address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"miaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"miaaddress\"  (string, required) The mia address to use for the signature.\n"
