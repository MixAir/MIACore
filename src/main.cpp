...
#if defined(NDEBUG)
#error "MIA cannot be compiled without assertions."
#endif
...
unsigned int dgpMaxBlockSize = 2000000; // mia
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            uint64_t minGasPrice = miaDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = miaDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            ...
            std::vector<LuxTransaction> miaTransactions = resultConverter.first;
            std::vector<EthTransactionParams> miaETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(LuxTransaction miaTransaction : miaTransactions){
                sumGas += miaTransaction.gas() * miaTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, miaTransaction.gasPrice());
                } else {
                    txMinGasPrice = miaTransaction.gasPrice();
                }
                VersionVM v = miaTransaction.getVersion();
                if(v.format!=0)
                ...
                if(miaTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(miaTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(miaTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)miaTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(miaETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > miaTransactions.size())
...
//#if 0
      if (pindex->nHeight > Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // mia
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // mia
...
#if 0
    LuxDGP miaDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setLuxSchedule(miaDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//miaDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//miaDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//miaDGP.getBlockSize(pindex->nHeight + 1);
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (pindex->nHeight >= Params().FirstSCBlock()) {
        ...
        try {
            oldHashStateRoot = dev::h256(globalState->rootHash()); // mia
        ...
        try {
            oldHashUTXORoot = dev::h256(globalState->rootHashUTXO()); // mia
        } catch (std::exception& e) {
            // When root node is empty, it will throw exception. We must re-initialize value
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (!rv) {
            ...
            if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
                globalState->setRoot(oldHashStateRoot); // mia
                globalState->setRootUTXO(oldHashUTXORoot); // mia
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 MIA
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path miaDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(miaDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(miaDir.string(), std::ios::in | std::ios::out);
...
bool LuxTxConverter::extractionLuxTransactions(ExtractLuxTX& miatx){
    ...
    miatx = std::make_pair(resultTX, resultETP);
    return true;
}
