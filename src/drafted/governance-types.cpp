...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "MIANetwork") type = MIANetwork;
    if(strType == "MIANetworkVariable") type = MIANetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MIANetwork) s = "MIANetwork";
    if(type == MIANetworkVariable) s = "MIANetworkVariable";
