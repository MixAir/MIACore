...
*Setup instructions for a gitian build of MIA using a Debian VM or physical system.*
...
Clone the git repositories for mia and gitian and then checkout the mia version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mia repository
...
To build MIA (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mia repository.
...
```bash
    ...
    From https://github.com/216k155/mia
...
For example:
```bash
URL=https://github.com/crowning-/mia.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mia-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mia-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mia-build.assert
```
...
[mia/gitian.sigs](https://github.com/216k155/gitian.sigs/) repository, or if that's not possible to create a pull
request.
