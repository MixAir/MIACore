...
*Setup instructions for a Gitian build of MIA Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MIA
...
- [Building MIA Core](#building-mia-core)
...
Clone the git repositories for MIA Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/MixAir/mia
```
...
in the MIA Core repository under 'Fetch and build inputs' to install sources which require
...
To build MIA Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MIA Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mia/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/MixAir/mia
...
For example:
```bash
URL=https://github.com/MixAir/MIACore.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mia=${COMMIT} --url mia=${URL} ../mia/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mia=${COMMIT} --url mia=${URL} ../mia/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mia=${COMMIT} --url mia=${URL} ../mia/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mia git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mia/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/MixAir/mia-detached-sigs.git

BTCPATH=/some/root/path/MIACore.git
SIGPATH=/some/root/path/mia-detached-sigs.git

./bin/gbuild --url mia=${BTCPATH},signature=${SIGPATH} ../mia/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mia-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mia-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mia-osx-build.assert
```
...
[mia/gitian.sigs](https://github.com/MixAir/gitian.sigs/) repository, or if that's not possible to create a pull
request.
